...
pub fn help(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) {
    ...
    let text: Vec<Spans> = vec![
        ...
        Spans::from("Odpowiedzi zawsze będą w formie liczby (WAŻNE!)"),
        Spans::from("Po ukończeniu wszystkich poziomów w menu pojawi się opcja 'Nagroda'"),
        Spans::from(""),
...
pub fn reward(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) {
        ...
        let text: Vec<Spans> = vec![
            ...
            Spans::from("Zaczynamy o 13:00, przyjdź chwilę wcześniej"),
            Spans::from("Plan:"),
            Spans::from(" - strzelnica"),
            Spans::from(" - flippery + obiad tam (http://flipperstacja.pl/)"),
            Spans::from(" - kolacja (ok. 19:30 ) w restauracji (http://gdansk.crazybutcher.pl/)"),
            Spans::from(""),
            Spans::from("zobaczymy się na miejscu!"),
            Spans::from(""),
            Spans::from("(P.S. napisz do mnie, że otrzymałeś zaproszenie i potwierdzasz obecność)"),
            Spans::from(""),
